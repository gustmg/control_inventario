				...
				<div class="col s12 m6">
					<div class="card hoverable">
						...
						<div class="card-action center no-padding no-select scale-transition">
			            	...
			            	<a href="#updateWarehouseModal{{$warehouse->warehouse_id}}" class="modal-trigger btn-floating white z-depth-0 tooltipped" data-position="bottom" data-delay="100" data-tooltip="Editar usuario">
			            		<i class="material-icons black-text" style="opacity: .6;">edit</i>
			            	</a>
			            </div>
					</div>
			    </div>
			    <div id="updateWarehouseModal{{$warehouse->warehouse_id}}" class="modal updateWarehouseModal">
			    	<div class="modal-content">
			    		<div class="row">
			    			<div class="col s12">
			    				<h5>Editar almacén</h5>
			    			</div>
			    			<form id="updateWarehouseForm{{$warehouse->warehouse_id}}" class="col s12 no-padding updateWarehouseForm" method="POST" action="warehouses/{{$warehouse->warehouse_id}}">
			    				{{ csrf_field() }}
			    				@method('PUT')
			    				<div class="row" style="margin-bottom: 10px;">
			    					<div class="col s12 grey-text text-darken-2"><b>Información general</b></div>
			    					<div class="input-field col s12 m8">
			    						<input id="warehouse_name" name="warehouse_name" type="text" class="validate warehouse_name" onblur="validateForm();" value="{{$warehouse->warehouse_name}}" required>
			    						<label for="warehouse_name" data-error="Verifique este campo" data-success="Campo validado">Nombre del almacén *</label>
			    			        </div>
			    			        <div class="input-field col s12">
			    						<input id="warehouse_address" name="warehouse_address" type="text" class="warehouse_address validate" onblur="validateForm();" value="{{$warehouse->warehouse_address}}" required>
			    	...
			    	<div class="modal-footer">
			    		...
			    		<button id="submit_button" onclick="submitUpdateWarehouse({{$warehouse->warehouse_id}});" class="modal-action btn waves-effect submit_button" disabled><b>Editar</b></button>
			    	</div>
			    </div>
			@endforeach
		@endif
